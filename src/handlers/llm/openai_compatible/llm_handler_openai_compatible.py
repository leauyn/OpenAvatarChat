...
from handlers.llm.openai_compatible.tools import tools, get_user_info as tool_get_user_info, get_user_survey_data as tool_get_user_survey_data, query_knowledge_base as tool_query_knowledge_base, get_guidance_plan as tool_get_guidance_plan, get_guidance_by_dimension as tool_get_guidance_by_dimension
...
def execute_tool_call(tool_call, context=None):
    ...
    elif function_name == "get_guidance_by_dimension":
        user_id = function_args.get("user_id", "")
        dimension_name = function_args.get("dimension_name", "")
        logger.info(f"📋 根据维度获取指导方案，用户ID: {user_id}, 维度: {dimension_name}")
        result = tool_get_guidance_by_dimension(user_id, dimension_name)
    else:
